    ...
    for(it = allFuncs.begin(); it != allFuncs.end(); it++){
        ...
        for(it2 = ++itTmp; it2 != allFuncs.end(); it2++){
            ...
            if(support <= funCalls[combo].size()) {
                double confit1 = ((double)(100*funCalls[combo].size())/funCalls[*it ].size());
                double confit2 = ((double)(100*funCalls[combo].size())/funCalls[*it2].size());
                if(confit1 > confidence && confit1 < 100) {
                    for(it3 = funCalls[*it ].begin(); it3 != funCalls[*it ].end(); it3++) {
                        if(!funCalls[combo].count(*it3)) {
                            cout << "bug: " << *it  << " in " << *it3 << ", pair: (" << *it << ", " << *it2 << "), support: " << funCalls[combo].size() << ", confidence: " << fixed << setprecision(2) << confit1 << "%\n";
                        }
                    }
                }
                if(confit2 > confidence && confit2 < 100) {
                    for(it3 = funCalls[*it2].begin(); it3 != funCalls[*it2].end(); it3++) {
                        if(!funCalls[combo].count(*it3)) {
                            cout << "bug: " << *it2 << " in " << *it3 << ", pair: (" << *it << ", " << *it2 << "), support: " << funCalls[combo].size() << ", confidence: " << fixed << setprecision(2) << confit2 << "%\n";
                        }
                    }
                }
            }
